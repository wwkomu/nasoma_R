...
# Introduction
This exercise attempts to visualise the global life expectancy and conduct a variety of statistical tests to compare the life expectancy of among a select countries. The primary purpose is to compare how life expectancy in Kenya has changes over the years!!

## Loading packages
Like any other R-related exercise, it is a prerequisite that we load the various packages that are going to enable the functionality that we need to successfully conduct this exercise! Like usual, I will load the <font color="dodgerblue">`tidyverse`</font>.
...
### Loading packages
The chuck below loads the packages that I need or could need as I take on this exercise!
```{r load_pkgs}
library(tidyverse)
library(janitor)
library(ggthemes)
...
I had copied two color palette, which supposedly show empathy to those with color blindness, from [this site](example.com)! They are based on a few colors with a base of `gray` and `black`! I went on and saved them as snippets.
...
```{r loads_data}
my_gapminder <- read_csv("/home/iamlnx/Documents/GitHub/nasoma_R/global_health_indicators/data/LifeExp_WorldBank.csv", skip = 4) %>%
...
This one is not a refined copy. I have hidden the code that generates it!
...
animate(ke_p, width = 6.5, height = 4.5, unit = "in", res = 300)

anim_save("figures/climate_spiral_animated_nasa.gif")
```
...
my_gapminder %>%
    ...
    labs(
        ...
        subtitle = "Kenya's versus other East African countries",
        x = "Year",
...
ggsave("figures/cbb_global_life_exp_Kenya_vs_others_1960-2020.png", width = 4.5, height = 4.5, unit = "in")
```
...
ggsave("figures/cb_global_life_exp_Kenya_vs_others_1960-2020.png", width = 4.5, height = 4.5, unit = "in")
```
...
# UPDATES
Game ya #TeamKubwa imeweza kuweza!!!! <font color = "darkred">#COYG</font>. No wonder, **<font color = "darkred">LONDON</font>** is **<font color = "darkred">RED</font>**!!
```{r}
